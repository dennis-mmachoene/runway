...
create type category as enum (
  'groceries', 'eating_out', 'transport', 'bills', 'shopping',
  'health', 'entertainment', 'cash', 'other'
);
...
create index idx_commitments_user     on commitments (user_id) where is_active;
...
create policy "own rows" on income_events
  for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "own rows" on commitments
  for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "own rows" on cycles
  for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "own rows" on transactions
  for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "own rows" on merchant_aliases
  for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "own row" on settings
  for all using (user_id = auth.uid()) with check (user_id = auth.uid());
